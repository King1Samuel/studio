...
const extractContentPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert at parsing job descriptions from web pages.
    ...
    Scour the entire document, including script tags which may contain the job data in JSON format, and find the specific, full job description for ONLY the "{{{role}}}" role. Extract the entire description, including requirements, responsibilities, qualifications, etc.
